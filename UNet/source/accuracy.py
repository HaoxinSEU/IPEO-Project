...
def count_for_user_accuracy(pred, target, cls, ignored_class = 2):
...
def count_for_producer_accuracy(pred, target, cls):
    pred = pred.view(-1)
    target = target.view(-1)
...
def count_for_overall_accuracy(pred, target, ignored_class = 2):
    pred = pred.view(-1)
    target = target.view(-1)
    ...
    pred_inds[target==ignored_class] = 0
